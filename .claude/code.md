...
**Сеть у PlayerController — через композицию, не наследование.** `Controller` (база с possession) —
`MonoBehaviour`, её наследует и host-only `ScriptController` (AI), которому `NetworkObject` не
нужен; поэтому `Controller` сетевым делать нельзя. `PlayerController` остаётся `: Controller`, а
сетевую идентичность даёт отдельный компонент-адаптер `PlayerNetwork : NetworkBehaviour` на том же
объекте: (1) `CheckObjectVisibility` — объект виден только владельцу (+ хост-сервер всегда),
(2) включает `PlayerController` лишь у владельца (`IsOwner`) — иначе на хосте копии контроллеров
чужих игроков читали бы локальный ввод. Отдельный `NetworkObject` от `PlayerState` (у того
видимость для всех). Owner-side привязка камеры/призрака в сетевом потоке — на сборке сессии.
